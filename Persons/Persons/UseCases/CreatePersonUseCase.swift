...
struct CreatePersonUseCase {
    ...
    func prepare() -> Single<ValidatedResult<Person, PersonForm>> {
        ...
        let addToRepository = prepareAddToRepository(repository)
        
        return Single
            .just(unvalidated)
            .map(validate)
            .flatMap(addToRepository)
    }
    ...
    private func prepareAddToRepository(_ repository: PersonRepository) -> (_ unvalidated: UnvalidatedPerson, _ result: ValidatedResult<ValidatedPerson, [PersonValidationKey: PersonValidationError]>) -> Single<ValidatedResult<Person, PersonForm>> {
